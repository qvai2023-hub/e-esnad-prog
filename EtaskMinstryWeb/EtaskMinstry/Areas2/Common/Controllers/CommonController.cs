...
using EtaskMinstry.Models.TaskCommon;


namespace EtaskMinstry.Areas.Common.Controllers
{
    public class CommonController : BaseCommonController
    {
        ...
        public ActionResult Index(int s = 0, bool? isNotAssign = null, bool? isArchive = null)
        {
            ...
            ViewBag.Employee = new SelectList(EtaskMinstry.AppCode.ServiceManger.GetCompanyEmployeeNotDeleted(MvcApplication.userData.CompanyId.Value), "id", "name");
        ...
        {
            var Task =new TaskAddEditCommon().gettask(int.Parse(id));
            ViewBag.TimeUnit = new SelectList(new EtaskMinstry.Models.TimeUnit.TimeUnitDisplay().Get(), "ID", "Name");
            ViewBag.Emplyees = new SelectList(EtaskMinstry.AppCode.ServiceManger.GetCompanyEmployeeNotDeleted(MvcApplication.userData.CompanyId.Value), "id", "name");
            ViewBag.IsEditEnable = TaskManger.CanEditTask(int.Parse(id));
            int? projId = Task.ProjectID;
            if (projId != null)
            ...
            if (!TaskManger.IsValidTask(id))
                ...
                return View(new TaskDetailCommonVM().Select(int.Parse(id), isAttach == 1));
        }
        ...
        {
            ViewBag.IsEditEnable = TaskManger.CanEditTask(int.Parse(id));
            return PartialView(new TaskExtensionsCommonVM().Select(int.Parse(id)));
        }
                                     ...
                                     TaskStatus iStatus = 0, int EmpID = 0, int PriorityID = 0, int ProjectID = 0)
        {
            ...
            return PartialView("PartialTaskCommon", new TaskCommonVM().Select(strTitle, FromStartDate, ToStartDate,
