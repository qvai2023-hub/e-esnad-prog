...
namespace EtaskMinstry.Models.Task
{
    public class TaskAddEdit
    {
        ...
        [RegularExpression(ValidationResource.RevString150, ErrorMessageResourceName = "ValidRegularFullName150",
            ErrorMessageResourceType = typeof (ValidationMessage.ValidationMessages))]
        //[Remote("checkDublicated", "Task", AdditionalFields = "TaskID",
        //    ErrorMessageResourceType = typeof (ValidationMessage.ValidationMessages),
        //    ErrorMessageResourceName = "DublicatedItem")]
        [Display(Name = "عنوان المهمة")]
        public String Title { get; set; }
        [Display(Name = "عنوان المهمة المختصر")]
        ...
        [Display(Name = "إسناد المهمة لـ")]
        public List<int?> EmployessLst { get; set; }

        // Company ID.
        public int CompanyID { get; set; }

        // Is Archived .
        [Display(Name = "نقل إلى الأرشيف")]
        ...
        public int Save()
        {
            ...
            else // Update Task 
            {
                ...
                if (objTask != null)
                {
                    ...
                    objTask.Summary = Summary;
                    objTask.TimeUnitID = TimeUnitID.HasValue? TimeUnitID.Value:(int?)null;
                    objTask.Title = Title;
                    objTask.BriefTaskName = BriefTaskName;
        ...
        public TaskAddEdit Select(object iID)
        {
            ...
            return new TaskAddEdit()
                {
                    ...
                    EmpID = objTask.EmpID.HasValue ? objTask.EmpID.Value:0,
                EndDate = objTask.EndDate.HasValue ? MvcApplication.IsGregDate ? objTask.EndDate.Value.ToGregDate3() : objTask.EndDate.Value.ToHijriDate() : "",
                ExpectedTime = objTask.ExpectedTime.HasValue ? objTask.ExpectedTime : null,
                    IsArchived = objTask.IsArchived,
                StartDate = objTask.StartDate.HasValue ? MvcApplication.IsGregDate ? objTask.StartDate.Value.ToGregDate3() : objTask.StartDate.Value.ToHijriDate() : "",
                StatusID = objTask.StatusID,
                    TimeUnitID = objTask.TimeUnitID.HasValue? objTask.TimeUnitID.Value:0,
                    Summary = objTask.Summary,
                    Title = objTask.Title,
                    TaskID = objTask.TaskID,
                    BriefTaskName=objTask.BriefTaskName,
       ...
       /// <returns></returns>
        public bool CheckDublication(int? TaskID, string Title)
        {
            return
                _unitOfWork.TaskRepository.Get(
                    ...
                    i.TaskID != TaskID
                    &&
                    i.CompanyID == MvcApplication.userData.userId
                    &&
                    !i.IsDeleted).Any();
        }
        ...
        /// <returns></returns>
        public List<TaskAddEdit> GetTasksByProject(int ProjectID)
        {
