...
namespace EtaskMinstry.Areas.Company.Models
{
    public class CompanyEmployeeVM
    {
        ...
        //[Remote("CheckDuplicate", "Employee", AdditionalFields = "EmpID", ErrorMessage = "الاسم موجود بالفعل")]
        [Display(Name = "الاسم")]
        ...
        public List<CompanyEmployeeVM> Get(String strName = "")
        {
            strName = strName.Trim().ToLower();
            List<CompanyEmployeeVM> objEmp = new List<CompanyEmployeeVM>();

            objEmp = _unitOfWork.Employee.Get(e => e.IsActive == true && e.IsDeleted == false).Select(e => new CompanyEmployeeVM()
                {
                    Id = e.EmpID,
                    Name = e.Name,
        ...
        public bool CheckForUniqueName(int? companyID, int? empID, string Name)
        {
            ...
            var result = _unitOfWork.Employee.Get(filter: e => e.CompanyID == companyID && e.EmpID != empID && e.Name.Contains(Name) && e.IsDeleted == false).Count() > 0 ? false : true;
            return result;
        }
        ...
        public bool CheckForUniqueEmail(int? companyID, string Email, int? empID = null)
        {
            ...
            var Empresult = _unitOfWork.Employee.Get(filter: e =>
        ...
        public bool CheckForUniqueMobile(int? companyID, int? EmpID, string Mobile)
        {
            ...
            var result = _unitOfWork.Employee.Get(filter: e => (e.CompanyID == companyID && e.EmpID != EmpID && e.Mobile.Contains(Mobile) && e.IsDeleted == false) || (e.CompanyID != companyID && e.Mobile.Contains(Mobile) && e.IsDeleted == false)).Count() > 0 ? false : true;
            return result;
        }
        public bool CheckForUniqueNationalID(int? companyID, int EmpID, string NationalID)
        {
            var result = _unitOfWork.Employee.Get(filter: e =>
                ...
                && e.NationalID.Contains(NationalID) && e.IsDeleted == false)
        ...
        public List<CompanyEmployeeVM> Select(int CompanyId, string Name, bool? bGender, string JobTitle, bool? bIsActive, string Email)
        {
            ...
            objEmp = _unitOfWork.Employee.Get(e => e.CompanyID == CompanyId && e.Name.Contains(Name) && e.JobTitle.Contains(JobTitle) && e.Email.Contains(Email)
        ...
        public CompanyEmployeeVM EmployeeDetails(int EmployeeId)
        {
           ...
           // empObj.Birthdate = (objEmp.Birthdate.HasValue) ? (MvcApplication.IsGregDate) ? objEmp.Birthdate.Value.ToGregDatediff() : objEmp.Birthdate.Value.ToHijriArabicDate() : String.Empty;
            ...
            empObj.LaborOfficeID = objEmp.LaborOfficeID;
            empObj.SequenceNumber = objEmp.SequenceNumber;
        ...
        public void SetIsActive(int? EmployeeId)
        {
            ...
            Generallog.Log(EmployeeId.Value, objEmp, objbefore, true, objEmp.Name, objEmp.EmpID, MvcApplication.userData.userId);
        }
        ...
        public Boolean Save()
        {
            ...
            using (TransactionScope tsTransScope = new TransactionScope())//(TransactionScopeOption.Suppress))
            {
                ...
                if (Id == 0)
                {
                    ...
                    //log.
                    Generallog.Log(Id, obj, null, true, Name, 0, MvcApplication.userData.userId);
                ...
                else
                {
                 ...
                 var updatesend=       QvLib.QVMail.SendMail(settingObj.ServerName, settingObj.UserName, settingObj.Password,
                    ...
                    bReuslt = true;
                    //log.
                    Generallog.Log(Id, obj, objbefore, true, Name, 0, MvcApplication.userData.userId);
                }
